...
import copy
import functools
from inspired.ec import archivers
from inspired.ec import generators
from inspired.ec import migrators
from inspired.ec import observers
from inspired.ec import replacers
from inspired.ec import selectors
from inspired.ec import terminators
from inspired.ec import variators
import itertools
import logging
import math
import time
...
'''def plotfun(mat,iterac, p = []):
    for i in range(len(mat)-1):
        for j in range(i+1,len(mat)):
            plt.title(f"Espacio de los objetivos en la iteración ojete {iterac}")
            plt.xlabel(f"f{i+1}(x)")
            plt.ylabel(f"f{j+1}(x)")
            if p == []:
                print('joj0')
                plt.plot(mat[i],mat[j],'.',color='red')
                plt.show()
            else:
                plt.plot(mat[i],mat[j],'.',color='red')
...
class EvolutionaryComputation(object):
    ...
    def __init__(self, random):
        ...
        self.logger = logging.getLogger('inspired.ec')
        try:
    ...
    def plotfun(m1,m2 = []):
        for i in range(len(m1)-1):
            for j in range(i+1,len(m1)):
                plt.title("Espacio de los objetivos chidos")
                plt.xlabel(f"f{i+1}(x)")
                plt.ylabel(f"f{j+1}(x)")
                '''plt.plot(m1[i],m1[j],'.')
                if m2 == []:
        ...
        while not self._should_terminate(list(self.population), self.num_generations, self.num_evaluations):
            ...
            if plot_iter == True:
                ...
                if obj_space != [] and iterac%step == 0 and iterac >= greater_than:
                    self.plotfun(evalf, iterac, obj_space)
                elif obj_space == [] and iterac%step == 0 and iterac >= greater_than:
                    self.plotfun(evalf, iterac)
            iterac += 1
            ###################################################################################
        ################
        if plot_iter == 'last':
            ...
            if obj_space == []:
                ...
                #plotfun(obj_space, iterac, evalf)
                self.plotfun(evalf, iterac, obj_space)
        #################
