...
########################  Introducido por mi para graficar ########################
######################## en cada iteración del algoritmo   ########################
def reord(x):
    evalu = []
    for i in range(len(x[0])):
        evalu.append([j[i] for j in x])
    return evalu

import matplotlib.pyplot as plt
'''def plotfun(mat,iterac, p = []):
    for i in range(len(mat)-1):
        for j in range(i+1,len(mat)):
            plt.title(f"Espacio de los objetivos en la iteración {iterac}")
            plt.xlabel(f"$f{i+1}(x)$")
            plt.ylabel(f"$f{j+1}(x)$")
            plt.plot(mat[i],mat[j],'.')
            if p == []:
                plt.show()
            else:
                plt.plot(p[i],p[j],'.',color='red')
                plt.show()'''

'''def plotfun(m1,iterac, m2 = [], save = False, name = None):
    #args.setdefault('name', None)
    for i in range(len(m1)-1):
        for j in range(i+1,len(m1)):
            plt.title(f"{name} - Iteración {iterac}",fontname='Latin Modern Roman', fontsize=16)
            plt.xlabel(f"$f{i+1}(x)$",fontname='Latin Modern Roman', fontsize=16)
            plt.ylabel(f"$f{j+1}(x)$",fontname='Latin Modern Roman', fontsize=16)
            if m2 == []:
                plt.plot(m1[i],m1[j],'.',color='red',markersize=10)
            else:
                plt.plot(m2[i],m2[j],'.')
                plt.plot(m1[i],m1[j],'.',color='red',markersize=10)
            if save != False and name != None:
                plt.savefig(f"{save}_It{iterac}_f{i+1}f{j+1}.jpg")
            plt.show()'''
def plotfun(m1,iterac, m2 = [], save = False, name = None, functions = []):
    if len(functions) == 1:
        for i in range(len(m1)-1):
            for j in range(i+1,len(m1)):
                plt.title(f"{name} - Iteración {iterac}",fontname='Latin Modern Roman', fontsize=16)
        ...
        for i in range(len(m1)-1):
            for j in range(i+1,len(m1)):
                plt.title(f"{name} - Iteración {iterac}",fontname='Latin Modern Roman', fontsize=16)
                plt.xlabel(f"${functions[i].__name__}(x)$",fontname='Latin Modern Roman', fontsize=16)
                plt.ylabel(f"${functions[j].__name__}(x)$",fontname='Latin Modern Roman', fontsize=16)
                if m2 == []:
                    plt.plot(m1[i],m1[j],'.',color='red',markersize=10)
                else:
                    plt.plot(m2[i],m2[j],'.')
    ...
    def __call__(self, candidate, args):
        ...
        if self.lower_bound is None or self.upper_bound is None:
            ...
            for i, (c, lo, hi) in enumerate(zip(candidate, self.lower_bound, 
                                                self.upper_bound)):
                if bounded_candidate[i] < lo or bounded_candidate[i] > hi:
...
class EvolutionaryComputation(object):
    ...
    ############ Tiene la opción de imprimir en cada iteración el espacio de objetivos, y asignarle un paso para las impresiones de
    ############ las gráficas
    def evolve(self, generator, evaluator, pop_size=100, seeds=None, maximize=True, bounder=None, obj_space=[], plot_iter=False,
               step = 1, greater_than=0, arch_name =False, a_name=None, satime = False, functions = [], **args):
        ...
        while not self._should_terminate(list(self.population), self.num_generations, self.num_evaluations):
            ...
            ######################## en cada iteración del algoritmo   ########################
            if plot_iter == True:
                evalf = reord([self.population[i].fitness for i in range(len(self.population))])
                if obj_space != [] and iterac%step == 0 and iterac >= greater_than:
                    plotfun(evalf, iterac, obj_space,arch_name, a_name, functions = functions)
                elif obj_space == [] and iterac%step == 0 and iterac >= greater_than:
                    plotfun(evalf, iterac, save = arch_name, name = a_name, functions = functions)
            iterac += 1
            ###################################################################################
        ################
        if plot_iter == 'last':
