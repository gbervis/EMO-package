...
def stats_observer(population, num_generations, num_evaluations, args):
    ...
    .. note::
       ...
       This function makes use of the ``inspired.ec.analysis.fitness_statistics`` 
    ...
    """
    stats = inspired.ec.analysis.fitness_statistics(population)
...
def file_observer(population, num_generations, num_evaluations, args):
    ...
    'inspired-statistics-file-<timestamp>.csv', contains the basic
    ...
    'inspired-individuals-file-<timestamp>.csv', contains every individual
    ...
    try:
        ...
        statistics_file = open('inspired-statistics-file-{0}.csv'.format(time.strftime('%m%d%Y-%H%M%S')), 'w')
        args['statistics_file'] = statistics_file
    try:
        individuals_file = args['individuals_file']
    except KeyError:
        individuals_file = open('inspired-individuals-file-{0}.csv'.format(time.strftime('%m%d%Y-%H%M%S')), 'w')
...
class EmailObserver(object):
    ...
    - *subject* -- the subject of the email (default 'inspired observer report')
    ...
    def __init__(self, username, password, server, port=587):
        ...
        self.subject = "inspired observer report"
