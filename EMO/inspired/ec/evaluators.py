...
def evaluator(evaluate):
    """Return an inspired evaluator function based on the given function.
    ...
    def inspired_evaluator(candidates, args):
        fitness = []
        for candidate in candidates:
            fitness.append(evaluate(candidate, args))
        return fitness
    inspired_evaluator.single_evaluation = evaluate
    return inspired_evaluator
...
def parallel_evaluation_pp(candidates, args):
    ...
    - *pp_evaluator* -- actual evaluation function to be used (This function
      should have the same signature as any other inspired evaluation function.)
    ...
    - *pp_secret* -- string representing the secret key needed to authenticate
      on a worker node (default "inspired")
    ...
    try:
        ...
        raise 
    secret_key = args.setdefault('pp_secret', 'inspired')
    try:
